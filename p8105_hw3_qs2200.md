...
ggplot(brfss_spa, aes(x = year, y = n_obs, color = state))+
  ...
  viridis::scale_color_viridis(discrete=TRUE, name = "State", guide = guide_legend(nrow = 5))+
  theme_bw()+
  theme(legend.position = "bottom")
```

![](p8105_hw3_qs2200_files/figure-markdown_github/problem%201.3-1.png) The "spaghetti plot" shows the number of observations in each state from 2002 to 2010. Most of the states have a constant number of observations, but the observation number dramaticly increased in 2007 and 2010, dramaticly decreased in 2008.
...
![](p8105_hw3_qs2200_files/figure-markdown_github/problem%201.5-1.png) From the five panel plot, we can see the proportion of each response category over time. "Very good" have the highest proportion among all five responses, and "Poor" have the lowest. For most states, the distributions within each category remains relatively constant.
...
![](p8105_hw3_qs2200_files/figure-markdown_github/problem%202.3-1.png) This plot shows the number of items ordered in each aisle, and the aisles are classified by department. **4. Make a table showing the most popular item aisles “baking ingredients”, “dog food care”, and “packaged vegetables fruits”.**
...
![](p8105_hw3_qs2200_files/figure-markdown_github/problem%203.4-1.png) From the T-max vs T-min graph, we can see the relationship between tmax and tmin. The highest density appears in the middle of the plot.
